    ...
    <body>
      <div class="action-bar no-print">
        <div class="action-bar-title">
          📋 Vista Previa del Informe de Tensión Arterial (Servidor Local)
        </div>
      ...
      <div class="header">
        ...
        <div class="header-meta">
          ...
          <p style="margin:4px 0 0 0; color:#10b981; font-weight:600;">✓ Servidor Local Sincronizado</p>
        </div>
      </div>
      ...
      <div class="footer">
        Este documento ha sido generado de forma privada desde tu Servidor Local.
      </div>
    </body>
    </html>
  `;
...
function generateChartSVG(chronologicalSessions: BloodPressureSession[]): string {
  ...
  return `
    <svg viewBox="0 0 ${width} ${height}" style="width:100%; height:auto;">
      ...
      <!-- Puntos Sistólica -->
      ${sysPoints.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="2" fill="#ef4444" stroke="#ffffff" stroke-width="0.75" />`).join('')}

      <!-- Puntos Diastólica -->
      ${diaPoints.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="2" fill="#3b82f6" stroke="#ffffff" stroke-width="0.75" />`).join('')}

      <!-- Puntos Pulsaciones (Gris) -->
      ${pulsePoints.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="1.5" fill="#64748b" stroke="#ffffff" stroke-width="0.75" />`).join('')}
