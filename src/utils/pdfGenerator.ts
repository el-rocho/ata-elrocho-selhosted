...
export function printPDFReport(
  ...
  const html = `
    ...
    <body>
      ...
      <div class="footer">
        Generado desde tu Servidor Centralizado Local de Tensión Arterial.
      </div>
...
function generateChartSVG(chronologicalSessions: BloodPressureSession[]): string {
  ...
  return `
    <svg viewBox="0 0 ${width} ${height}" style="width:100%; height:auto;">
      ...
      ${sysPoints.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="3.5" fill="#ef4444" stroke="#ffffff" stroke-width="1.5" />`).join('')}
      ${diaPoints.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="3.5" fill="#3b82f6" stroke="#ffffff" stroke-width="1.5" />`).join('')}
      ${pulsePoints.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="3" fill="#ec4899" stroke="#ffffff" stroke-width="1.5" />`).join('')}
